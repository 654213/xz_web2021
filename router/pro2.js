...
r.get('/v1/code', (req, res) => {
    ...
    req.session.captcha = cap.text; // session 存储验证码数值 1+5 = 6
...
r.post("/v1/remove3", (req, res) => {
    ...
    num = num.slice(0, num.length - 1); //1,3,5
    console.log(num);
    var sql = "DELETE FROM xz_user WHERE uid in (" + num + ")"; //in(1,3,5)
...
var storage = multer.diskStorage({
    ...
    filename: function(req, file, cb) { //上传后文件名称->修改文件名
        ...
        //获取原来文件后缀   jpg  png  gif    1.jpg
        var ext = file.originalname.substr(fileName.length - 5);
        // 将保存文件名设置为 字段名 + 时间戳，比如 1-1478521468943.jpg
...
r.post('/v1/upload', upload.single('logo'), function(req, res, next) {
    ...
    pool.query("update xz_user SET  avatar = ? WHERE uid = ?", [file, uid], (err, result) => {
        if (err) throw err;
        res.send({ ret_code: '0' });
    })
